...
import com.ex.bug_tracker_security_basic_auth.dto.BugRequestDTO;
...
public class BugController {
    ...
    public ResponseEntity<Page<BugResponseDTO>> getAllBugs(
        ...
        return ResponseEntity.ok(bugService.searchBugsAdvanced(status, priority, assignee, project, search, pageable));
    }
    ...
    public ResponseEntity<Bug> createBug(@RequestBody BugRequestDTO bugRequest) {
        Bug bug = new Bug(null, bugRequest.getTitle(), bugRequest.getDescription(), bugRequest.getStatus(), 
                         bugRequest.getAssignee(), bugRequest.getProject(), bugRequest.getPriority());
        return ResponseEntity.ok(bugService.createBug(bug));
    }
    
    @PutMapping("/admin/{id}")
    public ResponseEntity<Bug> updateBug(@PathVariable Long id, @RequestBody BugRequestDTO bugRequest) {
        Bug bug = new Bug(null, bugRequest.getTitle(), bugRequest.getDescription(), bugRequest.getStatus(), 
                         bugRequest.getAssignee(), bugRequest.getProject(), bugRequest.getPriority());
    ...
    @PutMapping("/developer/{id}/status")
    public ResponseEntity<Bug> updateBugStatus(@PathVariable Long id, @RequestParam Status status) {
        return ResponseEntity.ok(bugService.updateBugStatus(id, status));
    }
    
    @GetMapping("/stats")
    public ResponseEntity<Map<String, Object>> getBugStatistics() {
        return ResponseEntity.ok(bugService.getBugStatistics());
    }
}
