...
export class BugComponent implements OnInit {
  ...
  constructor(private bugService: BugService, private messageService: MessageService, private confirmationService: ConfirmationService) {}
  ...
  createBug() {
    if (!this.newBugTitle.trim()) {
      this.messageService.add({severity:'warn', summary:'Warning', detail:'Please enter a bug title'});
      return;
    }
    
    if (!this.newBugDescription.trim()) {
      this.messageService.add({severity:'warn', summary:'Warning', detail:'Please enter a bug description'});
      return;
    }
    
    if (!this.newBugPriority) {
      this.messageService.add({severity:'warn', summary:'Warning', detail:'Please select a priority'});
      return;
    }
    
    if (!this.newBugProject.trim()) {
      this.messageService.add({severity:'warn', summary:'Warning', detail:'Please enter a project name'});
      return;
    }
    
    if (!this.newBugAssignee.trim()) {
      this.messageService.add({severity:'warn', summary:'Warning', detail:'Please enter an assignee'});
      return;
    }
    ...
    operation.subscribe({
      next: () => {
        this.messageService.add({severity:'success', summary:'Success', detail: this.editingBug ? 'Bug updated successfully!' : 'Bug created successfully!'});
        this.resetForm();
        this.loadBugs();
        this.loadStats();
        this.loading = false;
  ...
  onBugStatusToggle(event: {id: number, status: string}) {
    ...
    this.bugService.updateBugStatus(event.id, event.status).subscribe({
      next: () => {
        this.messageService.add({severity:'success', summary:'Success', detail:`Bug status updated to ${event.status.toLowerCase().replace('_', ' ')}`});
        this.loadStats();
        this.loading = false;
      },
      error: (error) => {
